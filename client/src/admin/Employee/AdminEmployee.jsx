    ...
    return <div className="flex flex-col h-screen bg-neutral-950 text-neutral-100">
        ...
        <div className="flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 p-4 overflow-y-auto">
                <h1 className="text-2xl font-semibold mb-4 text-blue-400">Admin Employee</h1>

                {/* Add New Employee */}
                <div className="flex gap-3 mb-4 justify-end">
                    <button className="px-3 py-1 bg-emerald-700 hover:bg-emerald-600 rounded text-sm" onClick={() => navigate("/admin-add-employee")}>
                        + Add Employee
                    </button>
                        ...
                        <div
                            ...
                            <div className="flex items-center gap-3 flex-1">
                                ...
                                <div className="grid grid-cols-4 gap-x-6 gap-y-1 flex-1">
                                    <p><span className="font-medium text-amber-200">EmpID:</span> {emp.empId}</p>
                                    <p><span className="font-medium text-amber-200">Name:</span> {emp.fullName}</p>
                                    <p><span className="font-medium text-amber-200">Email:</span> {emp.email}</p>
                                    <p><span className="font-medium text-amber-200">Phone:</span> {emp.phone}</p>
                                    <p><span className="font-medium text-amber-200">City:</span> {emp.city}</p>
                                    <p><span className="font-medium text-amber-200">DOB:</span> {emp.dob}</p>
                                    <p><span className="font-medium text-amber-200">Role:</span> {emp.role}</p>
                                    <p>
                                        <span className="font-medium text-amber-200">Status:</span>{" "}
                                        <span
                                            className={`px-2 py-0.5 rounded text-xs ${emp.isActive === "Active"
                            ...
                            <div className="flex gap-2 ml-4">
                                <button className="flex items-center gap-1 px-2 py-1 bg-sky-800 hover:bg-sky-700 rounded text-xs" onClick={() => navigate('/admin-update-employee')}>
                                    <Eye size={14} /> View
                                </button>
